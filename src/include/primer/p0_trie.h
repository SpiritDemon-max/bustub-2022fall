...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    auto rt = &root_;
    std::unique_ptr<TrieNode> *pre_node;
    for (char i : key) {
      pre_node = rt;
      if (!rt->get()->HasChild(i)) {
        rt = rt->get()->InsertChildNode(i, std::make_unique<TrieNode>(i));
      } else {
        rt = rt->get()->GetChildNode(i);
      }
    }
    ...
    auto new_node = std::make_unique<TrieNodeWithValue<T>>(std::move(**rt), value);
    pre_node->get()->RemoveChildNode(key[key.size() - 1]);
    pre_node->get()->InsertChildNode(key[key.size() - 1], std::move(new_node));
  ...
  bool Remove(const std::string &key) {
    ...
    auto rt = &root_;
    std::vector<std::unique_ptr<TrieNode> *> node_ve;
    node_ve.clear();
    node_ve.emplace_back(rt);
    ...
    rt->get()->SetEndNode(false);
    for (int i = node_ve.size() - 1; i > 0; i--) {
      if (!node_ve[i]->get()->HasChildren() && !node_ve[i]->get()->IsEndNode()) {
        node_ve[i - 1]->get()->RemoveChildNode(key[i - 1]);
      } else {
        break;
      }
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    auto rt = &root_;
    for (char i : key) {
      if (rt->get()->GetChildNode(i) == nullptr) {
        *success = false;
        latch_.RUnlock();
        return {};
      }
    ...
    auto last_node = dynamic_cast<TrieNodeWithValue<T> *>(rt->get());
